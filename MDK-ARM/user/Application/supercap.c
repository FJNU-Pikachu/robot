...
#define POWER_SET_DEFAULT               35.0f                       //上电默认功率
...
//        Show_Status(s_code);
        ...
        CAN_Send();
        //串口3发送采样数据，用于调试
        sprintf(uart2_send_str,
                "in_v=%2.3f,in_c=%2.3f,cap_c=%2.3f,cap_v=%2.3f,dcdc_curr=%2.3f,dcdc_v=%2.3f,out_v=%2.3f,out_c=%2.3f\r\n",
                samp.in_v, samp.in_c, samp.cap_c, samp.cap_v, control.dcdc_curr, samp.dcdc_v,samp.out_v,samp.out_c );
        HAL_UART_Transmit(&huart2, (uint8_t *) uart2_send_str, strlen(uart2_send_str), 20);
...
//    PID_init(&control.currout_loop, PID_DELTA, 0.5f, 0.03f, 1.0f, 150, 5, -200, -5);     //电流环
//    PID_init(&control.voltout_loop, PID_POSITION, 1.0f, 0.01f, 3.0f, 100, 25, -100, -25);      //电压环
//    PID_init(&control.powerin_loop, PID_POSITION, 0.35f, 0.035f, 3, 400, 50, -400, -50);       //输入功率环
}
